...
const Header = () => {
  ...
  return (
    <header>
      ...
      {/* Main Navigation */}
      <div
        className={`bg-white shadow-md ${isScrolled ? "fixed w-full top-0 left-0 z-50" : ""}`}
      >
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:hidden lg:flex lg:space-x-8 items-center">
              <Link
                to="/"
                className="text-gray-700 hover:text-tertiary text-lg font-medium transition-colors"
              >
                Home
              </Link>
              <Link
            ...
            {/* Mobile Navigation */}
            <div
              className={`fixed top-0 right-0 h-screen w-[80%] bg-white z-40 transform ${isMenuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out lg:hidden shadow-xl`}
            >
              {/* Logo in mobile menu */}
